...
def CouponCode(request):
     ...
     if not coupon_obj.exists():
      message.warning(request,'Invalid Coupon.')
      return render(request,'cart.html',d)

     if cart_obj.coupon:
         message.warning(request, 'Coupon already exists.')
         return HttpResponseRedirect(request.META.get('HTTP_REFERER'))


     if cart_obj.get_cart_total() < coupon_obj[0].minimum_amount:
          message.warning(request, f'Amount should be greater than {coupon_obj[0].minimum_amount}')
          return HttpResponseRedirect(request.META.get('HTTP_REFERER'))


     if coupon_obj[0].is_expired:
         message.warning(request, f'Coupon expired.')
         return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
def remove_coupon(request_,card_id):
    cart = Cart.objects.get(uid = cart-id)
    cart.coupon = None
    cart.save()
...
def booking_detail(request,pid,bid,couponcode):
    if not request.user.is_authenticated:
        return redirect('login')
    user = User.objects.get(id=request.user.id)
    profile = Profile.objects.get(user=user)
